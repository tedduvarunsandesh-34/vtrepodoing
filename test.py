...
class Mirror(TaskListener):
    def __init__(self,client, message, is_qbit=False, is_leech=False, is_jd=False, is_nzb=False, vidMode=None, same_dir=None, bulk=None, multi_tag=None, options=""):
        if same_dir is None:
            same_dir = {}
        if bulk is None:
            bulk = []
    ...
    async def new_event(self):
        ...
        if check_msg:
            await delete_links(self, self.message)
            await send_message(self.message, check_msg, check_button, pin=False, delete=3600, both=True)
            return
        args = {
            "-doc": False,
            "-med": False,
        ...
        if Config.DISABLE_MULTI and int(args.get("-i", 1)) > 1:
            await send_message(self.message, "Multi-downloads are currently disabled. Please try without the -i flag.")
            return

        if Config.DISABLE_SEED and args.get("-d", False):
            await send_message(self.message, "Seeding is currently disabled. Please try without the -d flag.")
            return
        ...
        if not is_bulk:
            ...
            if vidTool and not self.vidMode and self.same_dir:
        ...
        if vidTool and (not self.vidMode or not self.same_dir):
            self.vidMode = await SelectMode(self).get_buttons()
            if not self.vidMode:
        ...
        if is_telegram_link(self.link):
            try:
                reply_to, session = await get_tg_link_message(self.link, self.user_id)
            except Exception as e:
